function [TEXT, nameFile]=ReportPV(PlantName,Data)
...
if LastIrrTime ==LastPowTime
    ...
    TextTable1=createTable("PV","Now",string(datetime(LastIrrTime,'Format','HH:mm')),string(LastIrrValue),string(LastPowValue),string(PR),0);
    TextTable2=createTable("PV","Today",string(datetime(LastIrrTime,'Format','dd/MM/uuuu')),string(Eirr),string(Eprod),string(PR_day),0);
...
[nameFile,L,H]=createPlot(Data,PlantName,"PV");
